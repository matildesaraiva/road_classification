...
img_width = 64
dataset_path = 'data'
test_dataset_path = 'C:/Users/LENOVO/Desktop/thesis/data/misclassified/'test_balanced'
...
base_folder = 'test_balanced'
folders = ['no_road', 'road']
...
for folder_name in folders:
    ...
    for filename in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path):
            ...
            groundtruth = 0 if label == 'no_road' else 1
            img = image.load_img(file_path, target_size=(64, 64))
            img_array = image.img_to_array(img)
            img_array = np.expand_dims(img_array, axis=0)
            img_tensor = tf.convert_to_tensor(img_array, dtype=tf.float32)
            prediction = np.round(model.predict(img_tensor))[0][0].astype(int)
            classifications["file_name"].append(file_name)
            classifications["groundtruth"].append(groundtruth)
            classifications["prediction"].append(prediction)
...
# Run the test on all data for evaluation metrics
...
# Check the elapsed time to run the whole pipeline
